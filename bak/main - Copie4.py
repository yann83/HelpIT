...
debug_print(f"log dir {LOG_PATH}")
...
class NetworkDrive:
    ...
    def mount(self, remote_path, username, password):
        ...
        if not drive_letter:
            logging.error("No letter avalaible")
            return None
        else:
            debug_print(f"NET USE {drive_letter} {remote_path} ####### /user:{username} /persistent:no")
            command = f"NET USE {drive_letter} {remote_path} {password} /user:{username} /persistent:no"
            try:
                if username and password:
                    logging.info(f"Trying to mount: {remote_path} on {drive_letter}")
                    ...
                    if result.returncode == 0:
                        ...
                        logging.info(f"Successful mount drive on : {drive_letter}")
                        return drive_letter
                    else:
                        logging.error(f"Error mount : {result.stderr}")
                        return None

            except Exception as e:
                logging.error(f"Error fail to use NET USE : {e}")
    ...
    def unmount_all(self):
        ...
        if not self.mounted_drives:
            logging.info("No drive to unmount")
            return

        for drive in list(self.mounted_drives.keys()):
            try:
                debug_print(f'NET USE {drive}: /delete /y')
    ...
    def count_free_drives(self):
        """Count the number of available readers"""
        if sys.platform == 'win32':
            import string
            return 26 - 3 - len(self.mounted_drives)  # 26 letters - A,B,C - are no use
...
class HelpITGUI:
    ...
    def __init__(self):
        ...
        # Starting log
        logging.info(f"Start HelpIT by {self.admin_id}")
    ...
    def _load_config(self):
        ...
        if CONFIG_JSON.exists():
            ...
            if not self.password:
                debug_print("Access denied or cancelled")
                sys.exit(1)
        ...
        # self.admin_id is a tk.StringVar() not a string so set is used
    ...
    def create_default_config(self, file_path: Path) -> None:
        ...
        if result:
            debug_print(f"Password successfully created, app exit now ")
            sys.exit(1)
        else:
            debug_print("Abord")
    ...
    def _check_password(self):
        ...
        if result:
            debug_print("Password successfully updated")
        else:
            debug_print("Abord")
    ...
    def validate_target(self):
        ...
        try:
            ...
            if ping_ms >= PING_THRESHOLD:
                color = 'orange'
                logging.warning(f"Slow ping with {target}: {ping_ms}ms (threshold : {PING_THRESHOLD}ms)")
            else:
                color = 'green'
    ...
    def change_password(self):
        ...
        if result:
            debug_print("Password update successfully !")
        else:
            debug_print("Abord update")
    ...
    def open_drive(self, drive_letter):
        ...
        try:
            ...
            logging.error(f"Error decrypt password : {e}")
            messagebox.showerror("Error", "Error passwor decrypt issue.")
            return

        # Check login
        username = self.admin_id.get()  # ← Utiliser .get() pour StringVar
        if not username or not self.admin_pwd:
            messagebox.showerror("Error", "Login missing.")
            return
        ...
        logging.info(f"Trying to connect with : {full_username}")
        ...
        try:
            ...
            if drive:
                os.startfile(drive)
                logging.info(f"Acces drive {drive_letter}: on {self.current_ip}")
                # Update counter
                self.label_drives.config(text=str(self.network_drives.count_free_drives()))
            else:
                messagebox.showerror("Error",
                                     f"Can't mount {drive_letter}$\n"
                                     f"Check:\n"
                                     f"- Login\n"
                                     f"- If administrative sharing C$ is enable\n"
                                     f"- If firefall allow this share")

        except Exception as e:
            logging.error(f"Error opening drive {drive_letter}: {e}")
            messagebox.showerror("Error", f"Counldn't assess {drive_letter}:\n{str(e)}")
    ...
    def on_closing(self):
        """Manage closing app"""
        self.network_drives.unmount_all()
        logging.info(f"HelpIT close by {self.admin_id.get()}")
        self.root.destroy()
